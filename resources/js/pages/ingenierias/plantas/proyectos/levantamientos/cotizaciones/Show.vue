...
interface LevantamientoResumen {
    id: number;
    folio: string;
}
...
function eliminar(): void {
    if (
        !confirm(
            `¿Eliminar la cotización "${props.cotizacion.folio}"? Esta acción no se puede deshacer.`,
        )
    ) {
        return;
    }
...
<template>
    ...
    <PageLayout
        title=""
        description=""
        endpoint="ingenierias.plantas.proyectos.levantamientos.cotizaciones"
    >
        <template #breadcrumbs>
            <Link
                :href="
                    LevantamientoController.show({
                        planta: planta.id,
        ...
        <div class="overflow-hidden rounded-2xl border shadow-sm">
            ...
            >
                ...
                <div class="flex flex-col items-end gap-2">
                    ...
                    <span
                        class="rounded-md px-3 py-1 text-xs font-bold uppercase"
                        :class="
            ...
            <div
                class="grid grid-cols-2 gap-6 border-b bg-card px-6 py-5 sm:grid-cols-4"
            >
                <div class="flex items-start gap-3">
                    <div
                        class="flex size-9 items-center justify-center rounded-lg bg-indigo-500/10 text-indigo-600"
                    >
                ...
                <div class="flex items-start gap-3">
                    <div
                        class="flex size-9 items-center justify-center rounded-lg bg-amber-500/10 text-amber-600"
                    >
                        <DollarSign class="size-4" />
                    </div>
                    <div>
                ...
                <div class="flex items-start gap-3">
                    <div
                        class="flex size-9 items-center justify-center rounded-lg bg-violet-500/10 text-violet-600"
                    >
                        <Clock class="size-4" />
                    </div>
                    <div>
                        ...
                        <p class="font-bold">
                            {{ formatoMoneda(cotizacion.costo_hora_total) }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid gap-4 border-b bg-card px-6 py-5 sm:grid-cols-2">
                <div class="rounded-lg bg-muted/40 p-4">
                    <p
                        class="mb-3 text-xs font-semibold text-muted-foreground uppercase"
                    >
                    ...
                    <dl class="space-y-2 text-sm">
                        ...
                        <div class="flex items-center gap-2">
                            <MapPin class="size-4 text-muted-foreground" /><span
                                class="font-medium"
                                >Dirección:</span
                            >
                ...
                <div class="rounded-lg bg-muted/40 p-4">
                    ...
                    <dl class="space-y-2 text-sm">
                        <div class="flex items-center gap-2">
                            <FileText
                                class="size-4 text-muted-foreground"
                            /><span class="font-medium">Obra:</span>
                            {{ cotizacion.obra ?? '—' }}
                        </div>
                        <div class="flex items-center gap-2">
                            <User class="size-4 text-muted-foreground" /><span
                                class="font-medium"
                                >Vendedor:</span
                            >
                            {{ cotizacion.vendedor ?? '—' }}
                        ...
                        <div class="flex items-center gap-2">
                            <Hash class="size-4 text-muted-foreground" /><span
                                class="font-medium"
                                >Proveedor:</span
                            >
        ...
        <div class="mt-6 rounded-2xl border bg-card p-6 shadow-sm">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div>
                    ...
                    <p class="font-medium">
                        {{ cotizacion.vigencia_cotizacion ?? '—' }}
                    </p>
                </div>
            </div>
        ...
        <div class="mt-6 rounded-2xl border bg-card p-6 shadow-sm">
            ...
            <div
                class="mb-5 flex items-center justify-between text-sm font-medium"
            >
                <span
                ...
                >
                    ...
                    >
                        <Button
                            ...
                            "
                            :variant="
                                cotizacion.tiene_insumos ? 'default' : 'outline'
                            "
                        >
                            Ver Insumos
                        </Button>
            ...
            <div
                class="mt-5 flex items-start gap-3 rounded-lg border px-4 py-3"
                :class="
                    cotizacion.completada
            ...
            >
                <component
                    :is="cotizacion.completada ? CheckCircle2 : Clock"
                ...
                <div>
                    ...
                    >
                        {{
                            cotizacion.completada
                                ? 'El proyecto puede continuar aunque el PDF de la OC se cargue después.'
                                : 'Puedes actualizar el estatus y cargar el PDF desde el panel de la OC.'
                        }}
                    </p>
                </div>
            </div>
            ...
            >
                ...
                >
                    ...
                    <span class="text-right">{{
                        formatoMoneda(hija.precioUnitario)
                    }}</span>
                    <span class="text-right font-medium">{{
                        formatoMoneda(hija.importe)
                    }}</span>
                </div>
            </div>

            <p
                v-if="!partidas.length"
